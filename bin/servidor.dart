...
void main() async {
  ...
  final router = Router()
    ...
    ..post('/prestar', _registrar)
    ..get('/solicitudes', _obtenerSolicitudes)
    ..post('/solicitudes/confirmar', _confirmar);
...
String _b64decode(String b64) {
  try {
    // base64Url.normalize agrega padding si falta
...
Response _estado(Request req) => Response.ok(
      jsonEncode({
        ...
        'solicitudesPendientes':
            _solicitudes.where((s) => s['procesado'] == false).length,
      }),
...
Response _formulario(Request req) {
  ...
  final tituloLegible =
      tituloB64.isNotEmpty ? _b64decode(tituloB64) : 'Libro de biblioteca';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { color: #7C6FCD; font-size: 17px; letter-spacing: 2px;
      text-align: center; margin-bottom: 6px; }
    .libro { color: #D4A853; font-style: italic; text-align: center;
      font-size: 15px; margin-bottom: 8px; padding: 8px 12px;
      background: rgba(212,168,83,0.08); border-radius: 8px;
      border-left: 3px solid #D4A853; }
    .aviso { color: #6A6A9A; font-size: 11px; text-align: center;
      margin-bottom: 20px; }
    label { display: block; color: #9E8FD8; font-size: 11px;
      letter-spacing: 1.5px; margin-bottom: 6px; margin-top: 18px; }
    input { width: 100%; padding: 13px 14px; background: #0F0F1A;
      border: 1px solid #3A3A5A; border-radius: 10px; color: #E8E8FF;
      font-size: 16px; outline: none; transition: border-color 0.2s; }
    input:focus { border-color: #7C6FCD; }
    button { width: 100%; padding: 15px; margin-top: 26px;
      background: linear-gradient(135deg, #7C6FCD, #5A4FAA); color: #fff;
...
<body>
  <div class="card">
    ...
    <form method="POST" action="/prestar">
      ...
      <input type="number" name="cedula" placeholder="Tu número de cédula"
...
Future<Response> _registrar(Request req) async {
  try {
    ...
    print('Nueva solicitud: $nombre (CC $cedula) → $titulo');
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { color: #5DBF85; font-size: 20px; letter-spacing: 2px;
      margin-bottom: 16px; }
    ...
    .aviso { margin-top: 20px; padding: 12px;
      background: rgba(93,191,133,0.08); border-radius: 8px;
      border: 1px solid rgba(93,191,133,0.2); color: #5DBF85;
      font-size: 12px; }
  </style>
</head>
<body>
  <div class="card">
    ...
    <div class="aviso">El bibliotecario recibirá tu solicitud en segundos
      y procesará el préstamo.</div>
  </div>
</body>
</html>''';
...
Response _obtenerSolicitudes(Request req) {
  final pendientes =
      _solicitudes.where((s) => s['procesado'] == false).toList();
  return Response.ok(
...
Future<Response> _confirmar(Request req) async {
  try {
    final body = await req.readAsString();
